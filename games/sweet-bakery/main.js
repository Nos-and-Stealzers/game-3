...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		//l('links').innerHTML=(Game.beta?'<a href="../" target="blank">Live version</a> | ':'<a href="beta" target="blank">Try the beta!</a> | ')+'<a href="http://orteil.dashnet.org/experiments/cookie/" target="blank">Sweet Bakery Classic</a>';
		l('links').innerHTML='<a href="http://orteil.dashnet.org/experiments/cookie/" target="blank">Sweet Bakery Classic</a>';
		...
		Game.WriteSave=function(exporting)//guess what we'e using to save the game?
		{
			...
			else
			{
				...
				str='SweetBakeryGame='+escape(str)+'; expires='+now.toUTCString()+';';
				document.cookie=str;//aaand save
				if (document.cookie.indexOf('SweetBakeryGame')<0) Game.Popup('Error while saving.<br>Export your save instead!');
		...
		Game.LoadSave=function(data)
		{
			...
			else if (document.cookie.indexOf('SweetBakeryGame')>=0) str=unescape(document.cookie.split('SweetBakeryGame=')[1]);//get cookie here
	...
	Game.Logic=function()
	{
		...
		if (Game.T%(Game.fps)==0) document.title=Beautify(Game.cookies)+' '+(Game.cookies==1?'cookie':'cookies')+' - Sweet Bakery Idle';
